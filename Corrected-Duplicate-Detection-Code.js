// DUPLICATE DETECTION & LOGGING - CORRECTED FOR "RUN ONCE FOR EACH ITEM" MODE
...
// Mode: "Run Once for Each Item" - uses $json instead of $input.all()

console.log('=== DUPLICATE DETECTION DEBUG - CORRECTED VERSION ===');
...
// ✅ ACCESS EXISTING DATA FROM MERGE NODE
// In "Run Once for Each Item" mode, we need to access previous node data differently
let existingData = [];

try {
  // Try to access Google Sheets Query data from the Merge node
  const mergeInputs = $('Merge').all();
  console.log(`📊 Merge node provided ${mergeInputs.length} items`);
  
  // Extract existing records (skip the first item which is current record)
  if (mergeInputs.length > 1) {
    existingData = mergeInputs.slice(1).map(item => item.json).filter(record => 
  ...
  console.warn('⚠️ Could not access Merge node data, trying alternative method:', error.message);
  
  // Fallback: Try to access Google Sheets Query directly
  try {
    const sheetsData = $('Google Sheets Query Existing Data').all();
    existingData = sheetsData.map(item => item.json).filter(record => 
      record && Object.keys(record).length > 0
    );
    console.log(`✅ Fallback: Found ${existingData.length} existing records from Google Sheets Query`);
  } catch (fallbackError) {
    console.warn('⚠️ Fallback also failed, treating as empty sheet:', fallbackError.message);
    existingData = [];
  }
}
...
const enhancedData = {
  ...
  processingNode: 'duplicate-detection-corrected',
  
  // ✅ ADD AUDIT TRAIL FIELDS
  processedAt: currentTimestamp,
  workflowName: 'LinkedIn-SEO-Gmail-sub-flow-Workshop-ContactTracking--Augment',
  nodeType: 'duplicate-detection-run-once-per-item',
