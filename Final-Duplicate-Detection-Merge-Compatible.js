// DUPLICATE DETECTION & LOGGING - MERGE NODE COMPATIBLE (FINAL VERSION)
...
// Position: After Merge node, before Google Sheets Tracking node

console.log('=== DUPLICATE DETECTION DEBUG - MERGE COMPATIBLE ===');

// ✅ ACCESS DATA FROM MERGE NODE
// Merge node combines: [0] = current record, [1+] = existing records
const allItems = $input.all();
console.log(`📊 Merge node provided ${allItems.length} items`);

// ✅ EXTRACT CURRENT RECORD (always first item from Merge)
const currentRecord = allItems[0].json;
console.log('✅ Current record keys:', Object.keys(currentRecord));

// ✅ EXTRACT EXISTING RECORDS (remaining items from Merge)
let existingData = [];
...
const enhancedData = {
  ...
  processingNode: 'duplicate-detection-merge-compatible',
  
  // ✅ ADD AUDIT TRAIL FIELDS
  processedAt: currentTimestamp,
  workflowName: 'LinkedIn-SEO-Gmail-sub-flow-Workshop-ContactTracking--Augment',
  nodeType: 'duplicate-detection-native-merge',
